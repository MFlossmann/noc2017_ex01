...
Jg = Function('Jg', {v,p}, {jacobian(g,v)});

% Insert you code here (create an NLP solver)
nlp = struct('x', v, 'f', J, 'g', g, 'p', p);
...
for i = 1:n_points
    ...
    sol = solver('x0', [1,1],...           % solution guess
                'lbg', [-inf,-inf,0],...    % lower bound on g
                'ubg', [0,0,0],...         % upper bound on g
                'lbx', [0,0],...            % lower bound on x
                'ubx', [inf, inf],...       % upper bound on x
                'p', alpha_val);
